import React from 'react';

export interface TextBulkProps {
  size?: number | string;
  color?: string;
  className?: string;
  style?: React.CSSProperties;
}

const TextBulk: React.FC<TextBulkProps> = ({ 
  size = 24, 
  color = "currentColor", 
  ...props 
}) => {
  return (
    <svg {...props} width={size} height={size} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
<path opacity="0.4" d="M22 7.81V16.19C22 19.83 19.83 22 16.19 22H7.81C7.61 22 7.41 21.99 7.22 21.98C5.99 21.9 4.95 21.55 4.13 20.95C3.71 20.66 3.34 20.29 3.05 19.87C2.36 18.92 2 17.68 2 16.19V7.81C2 4.37 3.94 2.24 7.22 2.03C7.41 2.01 7.61 2 7.81 2H16.19C17.68 2 18.92 2.36 19.87 3.05C20.29 3.34 20.66 3.71 20.95 4.13C21.64 5.08 22 6.32 22 7.81Z" fill={color}/>
<path d="M16.67 5.64001H7.33C6.18 5.64001 5.25 6.57001 5.25 7.72001V8.90001C5.25 9.31001 5.59 9.65001 6 9.65001C6.41 9.65001 6.75 9.31001 6.75 8.90001V7.72001C6.75 7.40001 7.01 7.14001 7.33 7.14001H11.25V16.86H9.47C9.06 16.86 8.72 17.2 8.72 17.61C8.72 18.02 9.06 18.36 9.47 18.36H14.54C14.95 18.36 15.29 18.02 15.29 17.61C15.29 17.2 14.95 16.86 14.54 16.86H12.76V7.14001H16.68C17 7.14001 17.26 7.40001 17.26 7.72001V8.90001C17.26 9.31001 17.6 9.65001 18.01 9.65001C18.42 9.65001 18.76 9.31001 18.76 8.90001V7.72001C18.75 6.58001 17.82 5.64001 16.67 5.64001Z" fill={color}/>
</svg>

  );
};

export default TextBulk;
