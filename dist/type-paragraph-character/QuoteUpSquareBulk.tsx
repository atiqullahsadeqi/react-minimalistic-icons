import React from 'react';

export interface QuoteUpSquareBulkProps {
  size?: number | string;
  color?: string;
  className?: string;
  style?: React.CSSProperties;
}

const QuoteUpSquareBulk: React.FC<QuoteUpSquareBulkProps> = ({ 
  size = 24, 
  color = "currentColor", 
  ...props 
}) => {
  return (
    <svg {...props} width={size} height={size} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
<path opacity="0.4" d="M16.19 2H7.82001C4.18001 2 2.01001 4.17 2.01001 7.81V16.18C2.01001 19.82 4.18001 21.99 7.82001 21.99H16.19C19.83 21.99 22 19.82 22 16.18V7.81C22 4.17 19.83 2 16.19 2Z" fill={color}/>
<path d="M15.81 7.21997H14.32C13.23 7.21997 12.38 8.06997 12.38 9.15997V10.65C12.38 11.74 13.23 12.59 14.32 12.59H16.23C16.15 14.4 15.73 14.67 14.52 15.38C14.16 15.59 14.05 16.05 14.26 16.41C14.4 16.65 14.65 16.78 14.91 16.78C15.04 16.78 15.17 16.75 15.29 16.68C17.08 15.62 17.75 14.93 17.75 11.85V9.17999C17.75 8.08999 16.88 7.21997 15.81 7.21997Z" fill={color}/>
<path d="M9.68 7.21997H8.19C7.1 7.21997 6.25 8.06997 6.25 9.15997V10.65C6.25 11.74 7.1 12.59 8.19 12.59H10.1C10.02 14.4 9.6 14.67 8.39 15.38C8.03 15.59 7.92 16.05 8.13 16.41C8.27 16.65 8.52 16.78 8.78 16.78C8.91 16.78 9.04 16.75 9.16 16.68C10.95 15.62 11.62 14.93 11.62 11.85V9.17999C11.61 8.08999 10.74 7.21997 9.68 7.21997Z" fill={color}/>
</svg>

  );
};

export default QuoteUpSquareBulk;
