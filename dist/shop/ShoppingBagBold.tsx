import React from 'react';

export interface ShoppingBagBoldProps {
  size?: number | string;
  color?: string;
  className?: string;
  style?: React.CSSProperties;
}

const ShoppingBagBold: React.FC<ShoppingBagBoldProps> = ({ 
  size = 24, 
  color = "currentColor", 
  ...props 
}) => {
  return (
    <svg {...props} width={size} height={size} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M16 8.75C15.59 8.75 15.25 8.41 15.25 8V4.5C15.25 3.42 14.58 2.75 13.5 2.75H10.5C9.42 2.75 8.75 3.42 8.75 4.5V8C8.75 8.41 8.41 8.75 8 8.75C7.59 8.75 7.25 8.41 7.25 8V4.5C7.25 2.59 8.59 1.25 10.5 1.25H13.5C15.41 1.25 16.75 2.59 16.75 4.5V8C16.75 8.41 16.41 8.75 16 8.75Z" fill={color}/>
<path d="M7.99997 17.78C7.58997 17.78 7.24997 17.44 7.24997 17.03C7.24997 16.61 7.58997 16.28 7.99997 16.28H19.76C20.06 16.28 20.29 16.02 20.26 15.72L19.58 10.03C19.34 8.09 19 6.5 15.6 6.5H8.39997C4.99997 6.5 4.65997 8.09 4.42997 10.03L3.52997 17.53C3.23997 19.99 3.99997 22 7.50997 22H16.49C19.65 22 20.58 20.37 20.53 18.25C20.52 17.98 20.3 17.78 20.03 17.78H7.99997Z" fill={color}/>
</svg>

  );
};

export default ShoppingBagBold;
