import React from 'react';

export interface MessageEditTwotoneProps {
  size?: number | string;
  color?: string;
  className?: string;
  style?: React.CSSProperties;
}

const MessageEditTwotone: React.FC<MessageEditTwotoneProps> = ({ 
  size = 24, 
  color = "currentColor", 
  ...props 
}) => {
  return (
    <svg {...props} width={size} height={size} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M16 2H8C4 2 2 4 2 8V21C2 21.55 2.45 22 3 22H16C20 22 22 20 22 16V8C22 4 20 2 16 2Z" stroke={color} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
<g opacity="0.4">
<path d="M12.9098 7.84003L7.71979 13.03C7.51979 13.23 7.3298 13.62 7.2898 13.9L7.0098 15.88C6.9098 16.6 7.40979 17.1 8.12979 17L10.1098 16.72C10.3898 16.68 10.7798 16.49 10.9798 16.29L16.1698 11.1C17.0598 10.21 17.4898 9.17003 16.1698 7.85003C14.8498 6.52003 13.8098 6.94003 12.9098 7.84003Z" stroke={color} strokeWidth="1.5" strokeMiterlimit="10" strokeLinecap="round" strokeLinejoin="round"/>
<path d="M12.1699 8.58002C12.6099 10.15 13.8399 11.39 15.4199 11.83" stroke={color} strokeWidth="1.5" strokeMiterlimit="10" strokeLinecap="round" strokeLinejoin="round"/>
</g>
</svg>

  );
};

export default MessageEditTwotone;
