import React from 'react';

export interface SearchZoomInBulkProps {
  size?: number | string;
  color?: string;
  className?: string;
  style?: React.CSSProperties;
}

const SearchZoomInBulk: React.FC<SearchZoomInBulkProps> = ({ 
  size = 24, 
  color = "currentColor", 
  ...props 
}) => {
  return (
    <svg {...props} width={size} height={size} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
<path opacity="0.4" d="M11.5 21C16.7467 21 21 16.7467 21 11.5C21 6.25329 16.7467 2 11.5 2C6.25329 2 2 6.25329 2 11.5C2 16.7467 6.25329 21 11.5 21Z" fill={color}/>
<path d="M21.3 22.0001C21.12 22.0001 20.94 21.9301 20.81 21.8001L18.95 19.9401C18.68 19.6701 18.68 19.2301 18.95 18.9501C19.22 18.6801 19.66 18.6801 19.94 18.9501L21.8 20.8101C22.07 21.0801 22.07 21.5201 21.8 21.8001C21.66 21.9301 21.48 22.0001 21.3 22.0001Z" fill={color}/>
<path d="M14.2 10.95H12.45V9.19995C12.45 8.78995 12.11 8.44995 11.7 8.44995C11.29 8.44995 10.95 8.78995 10.95 9.19995V10.95H9.20001C8.79001 10.95 8.45001 11.29 8.45001 11.7C8.45001 12.11 8.79001 12.45 9.20001 12.45H10.95V14.2C10.95 14.61 11.29 14.95 11.7 14.95C12.11 14.95 12.45 14.61 12.45 14.2V12.45H14.2C14.61 12.45 14.95 12.11 14.95 11.7C14.95 11.29 14.61 10.95 14.2 10.95Z" fill={color}/>
</svg>

  );
};

export default SearchZoomInBulk;
