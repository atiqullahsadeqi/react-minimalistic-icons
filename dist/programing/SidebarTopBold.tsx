import React from 'react';

export interface SidebarTopBoldProps {
  size?: number | string;
  color?: string;
  className?: string;
  style?: React.CSSProperties;
}

const SidebarTopBold: React.FC<SidebarTopBoldProps> = ({ 
  size = 24, 
  color = "currentColor", 
  ...props 
}) => {
  return (
    <svg {...props} width={size} height={size} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M20.9498 4.13C20.6598 3.71 20.2898 3.34 19.8698 3.05C18.9198 2.36 17.6798 2 16.1898 2H7.80978C4.36978 2 2.23979 3.94 2.02979 7.22H21.9798C21.8998 5.99 21.5498 4.95 20.9498 4.13Z" fill={color}/>
<path d="M2 16.19C2 17.68 2.36 18.92 3.05 19.87C3.34 20.29 3.71 20.66 4.13 20.95C5.08 21.64 6.32 22 7.81 22H16.19C19.83 22 22 19.83 22 16.19V8.72H2V16.19ZM8.91 14.44L11.47 11.88C11.76 11.59 12.24 11.59 12.53 11.88L15.09 14.44C15.24 14.59 15.31 14.78 15.31 14.97C15.31 15.16 15.24 15.35 15.09 15.5C14.8 15.79 14.32 15.79 14.03 15.5L12 13.48L9.97 15.5C9.68 15.79 9.2 15.79 8.91 15.5C8.62 15.21 8.62 14.74 8.91 14.44Z" fill={color}/>
</svg>

  );
};

export default SidebarTopBold;
