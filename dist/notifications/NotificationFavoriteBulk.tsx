import React from 'react';

export interface NotificationFavoriteBulkProps {
  size?: number | string;
  color?: string;
  className?: string;
  style?: React.CSSProperties;
}

const NotificationFavoriteBulk: React.FC<NotificationFavoriteBulkProps> = ({ 
  size = 24, 
  color = "currentColor", 
  ...props 
}) => {
  return (
    <svg {...props} width={size} height={size} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M19 8C20.6569 8 22 6.65685 22 5C22 3.34315 20.6569 2 19 2C17.3431 2 16 3.34315 16 5C16 6.65685 17.3431 8 19 8Z" fill={color}/>
<path opacity="0.4" d="M19 9.5C16.52 9.5 14.5 7.48 14.5 5C14.5 4.28 14.69 3.61 14.99 3H7.52C4.07 3 2 5.06 2 8.52V16.47C2 19.94 4.07 22 7.52 22H15.47C18.93 22 20.99 19.94 20.99 16.48V9.01C20.39 9.31 19.72 9.5 19 9.5Z" fill={color}/>
<path d="M10.9 12.36C10.33 12.18 9.71998 12.28 9.28998 12.62C8.84998 12.28 8.24998 12.18 7.68998 12.36C6.45998 12.76 6.06998 14.16 6.41998 15.24C6.94998 16.88 8.67998 17.73 9.29998 17.73C9.90998 17.73 11.67 16.86 12.18 15.24C12.53 14.16 12.13 12.76 10.9 12.36Z" fill={color}/>
</svg>

  );
};

export default NotificationFavoriteBulk;
