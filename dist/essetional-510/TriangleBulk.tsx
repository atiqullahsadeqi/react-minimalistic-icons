import React from 'react';

export interface TriangleBulkProps {
  size?: number | string;
  color?: string;
  className?: string;
  style?: React.CSSProperties;
}

const TriangleBulk: React.FC<TriangleBulkProps> = ({ 
  size = 24, 
  color = "currentColor", 
  ...props 
}) => {
  return (
    <svg {...props} width={size} height={size} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M12 2.59009V13.6101L2.65 20.1501C1.8 19.1401 1.75 17.6101 2.7 15.9001L5.82 10.2901L8.76 5.00009C9.65 3.40009 10.82 2.59009 12 2.59009Z" fill={color}/>
<path opacity="0.6" d="M21.3499 20.15C20.6999 20.94 19.5699 21.41 18.0599 21.41H5.9399C4.4299 21.41 3.2999 20.94 2.6499 20.15L11.9999 13.61L21.3499 20.15Z" fill={color}/>
<path opacity="0.4" d="M21.35 20.1501L12 13.6101V2.59009C13.18 2.59009 14.35 3.40009 15.24 5.00009L18.18 10.2901L21.3 15.9001C22.25 17.6101 22.2 19.1401 21.35 20.1501Z" fill={color}/>
</svg>

  );
};

export default TriangleBulk;
